...
class App extends Component {
  ...
  startDownload = async event => {
    ...
    await UserBase.methods
      .startDownload(
        this.state.downloadCost,
        this.state.fileLink,
        this.state.fileName,
        this.state.fileSize,
        this.state.rating,
        "0x0FcF01fCF8AF6f97BE17a4e1Db701D2cfFD78645",
        this.state.userId
      )
      .send({
        from: accounts[0],
  ...
  callUpload = async event => {
    ...
    await UserBase.methods
      .receiveReward(Math.floor(rewardAmount), this.state.userId)
      .send({
        from: accounts[0]
      });
  ...
  onClick = async event => {
            ...
            <List divided relaxed>
              <List.Item>
                ...
                <List.Content>
                  <List.Header>
                    {" "}
                    FileName: <span style={pStyle}>{searchTerm}</span>
                  </List.Header>
                </List.Content>
              </List.Item>
              ...
              <List.Item>
                ...
                <List.Content>
                  <List.Header>
                    Link of File: <span style={pStyle}>{element["link"]}</span>
                  </List.Header>
                </List.Content>
              </List.Item>
              ...
              <List.Item>
                ...
                <List.Content>
                  <List.Header>
                    Size of File:{" "}
                    <span style={pStyle}>{element["size"] * 100} Kb</span>
                  </List.Header>
                </List.Content>
              </List.Item>
              ...
              <List.Item>
                ...
                <List.Content>
                  <List.Header>
                    User Ratings:{" "}
                    <span style={pStyle}>{element["rating"]} (0-5)</span>
                  </List.Header>
                </List.Content>
              </List.Item>
              ...
              <List.Item>
                ...
                <List.Content>
                  <List.Header>
                    Cost of Downloading:{" "}
                    <span style={pStyle}>
                      {Math.floor((element["size"] * 5) / 3 * 100000000)} wei
                    </span>
                  </List.Header>
                </List.Content>
              </List.Item>
    ...
    this.setState({
      downloadCost: Math.floor(
        ((TorrentInfo[Randomindex]["size"] * 5) / 3) * 100000000
      )
    });
    this.setState({
      fileName: searchTerm,
      fileLink: TorrentInfo[Randomindex]["link"],
      fileSize: TorrentInfo[Randomindex]["size"],
      rating: TorrentInfo[Randomindex]["rating"],
      userId: accounts[0]
    });
      ...
      <Layout>
        <Container style={{ marginTop: "30px" }}>
          <Grid>
            ...
            <GridRow>{this.state.progress && <ProgressBar />}</GridRow>
          </Grid>
        </Container>
      </Layout>
    );
